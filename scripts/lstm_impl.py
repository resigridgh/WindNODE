...
def parse_args():
    parser = argparse.ArgumentParser(description="Train LSTM for wind turbine power prediction.")
    parser.add_argument("--data-path", type=str, default="data/T1.csv", help="Path to T1.csv")
    parser.add_argument("--output-dir", type=str, default="outputs/lstm", help="Directory to save outputs")
...
def main():
    ...
    data = load_and_preprocess_lstm_data(
    ...
    model = WindLSTMRegressor(input_dim=len(DEFAULT_FEATURE_COLS)).to(device)
    print(model)
    ...
    final_train_eval = evaluate_lstm_model(
        model=model,
    ...
    final_test_eval = evaluate_lstm_model(
        model=model,
    ...
    loss_pdf = output_dir / f"lstm_loss_curve_{ts}.pdf"
    ...
    matlab_like_axes(ax, xlabel_text=r"Epochs", ylabel_text=r"Loss", title_text="LSTM Training and Testing Loss")
    ax.legend()
    fig.tight_layout()
    fig.savefig(loss_pdf, format="pdf", bbox_inches="tight")
    plt.close(fig)

    acc_pdf = output_dir / f"lstm_accuracy_curve_{ts}.pdf"
    fig, ax = plt.subplots(figsize=(5, 2.5))
    ...
    matlab_like_axes(
        ...
        title_text="LSTM Training and Testing Accuracy",
    )
    ax.legend()
    fig.tight_layout()
    ...
    avp_pdf = output_dir / f"lstm_actual_vs_predicted_{ts}.pdf"
    fig, ax = plt.subplots(figsize=(5, 2.5))
    ...
    ax.plot([mn, mx], [mn, mx], color="red", linewidth=2, label="Ideal Line")
    matlab_like_axes(
        ax,
        xlabel_text=r"Actual Power (kW)",
        ylabel_text=r"Predicted Power (kW)",
        title_text="LSTM: Actual vs Predicted",
    )
    ax.legend()
    fig.tight_layout()
    ...
    comp_pdf = output_dir / f"lstm_power_prediction_comparison_{ts}.pdf"
    fig, ax = plt.subplots(figsize=(5, 2.5))
    ...
    matlab_like_axes(
        ...
        title_text="LSTM Wind Power Prediction",
    )
    ax.legend()
    fig.tight_layout()
    ...
    pth_file = output_dir / f"lstm_wind_power_{ts}.pth"
    torch.save(model.state_dict(), pth_file)
    print(f"\nSaved PyTorch weights: {pth_file}")

    onnx_file = output_dir / f"lstm_wind_power_{ts}.onnx"
    export_lstm_onnx_model(
        model=model,
        onnx_path=onnx_file,
        seq_len=args.seq_len,
    ...
    x_scaler_file = output_dir / f"lstm_x_scaler_{ts}.pkl"
    y_scaler_file = output_dir / f"lstm_y_scaler_{ts}.pkl"
